class Song < ApplicationRecord
  ...
  def self.schedule_post(song, time, current_user)
	...
	image_link = TEMP_IMAGE_HOST + Rails.application.routes.url_helpers.rails_blob_path(song.pic, only_path: true)
...
# subhead, @song.video, @song.score, @song.controversy, @song.reviews
  ...
  def self.generate_html(song, subhead, image_link)
    post_html = "<p><i>#{subhead}</i></p><center><p><img src= '#{image_link}' border = 2><b>[<a href='#{song.video}'>Video</a>]<BR><a title='Controversy index: #{sprintf('%.2f', song.controversy)}'>[#{sprintf('%.2f', song.score)}]</a></b></center></p>"
